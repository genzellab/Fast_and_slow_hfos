...
%Find PFC spindles
[spindle,SpinFreq,spin_duration,Mx_cortex,timeasleep,sig_cortex,Ex_cortex,Sx_cortex,...
  spindle_multiplets_cortex,SpinFreq_multiplets_cortex,spin_duration_multiplets_cortex,sig_multiplets_cortex,~, ...
  ]=gui_findspindlesYASA(CORTEX,states,xx,multiplets,fn);

%Extract spindle signal waveforms
...
[x,y,z,~,~,~,l,p]=specs_spindles(si,timeasleep,fn,print_hist); %Computes features
if print_hist==1
cd ..
...
%% PFC spindles
    spindles_cortex(k)=spindle;
    spindles_cortex_rate(k)=SpinFreq;
    spindles_cortex_duration(k)=spin_duration;
    clear spindle SpinFreq

%Multiplets    
for ll=1:3
   eval(['spindles_cortex_' multiplets{ll} '(k)=spindle_multiplets_cortex.' multiplets{ll} ';']) 
   eval(['spindles_cortex_rate_' multiplets{ll} '(k)=SpinFreq_multiplets_cortex.' multiplets{ll} ';']) 
   eval(['spindles_cortex_duration_' multiplets{ll} '(k)=spin_duration_multiplets_cortex.' multiplets{ll} ';'])    
end
...
[spindle,SpinFreq,spin_duration,Mx_par,timeasleep,sig_par,Ex_par,Sx_par,...
  spindle_multiplets_par,SpinFreq_multiplets_par,spin_duration_multiplets_par,sig_multiplets_par,Mx_multiplets_par...    
...
print_hist=0;
[x,y,z,~,~,~,l,p]=specs_spindles(si,timeasleep,fn,print_hist);
if print_hist==1
    cd ..
...
%% PAR spindles
spindles_par(k)=spindle;
spindles_par_rate(k)=SpinFreq;
spindles_par_duration(k)=spin_duration;

%Multiplets    
for ll=1:length(multiplets)
   eval(['spindles_par_' multiplets{ll} '(k)=spindle_multiplets_par.' multiplets{ll} ';']) 
   eval(['spindles_par_rate_' multiplets{ll} '(k)=SpinFreq_multiplets_par.' multiplets{ll} ';']) 
   eval(['spindles_par_duration_' multiplets{ll} '(k)=spin_duration_multiplets_par.' multiplets{ll} ';'])    
end
% xo
%% Co-occurent spindles
...
[x,y,z,w,h,q,l,p]=specs_spindles(Sig_par,timeasleep,fn,0);
fi_cohfo_par(k)=x;
fa_cohfo_par(k)=y;
amp_cohfo_par(k)=z;
count_cohfo_par(k)=w;
...
[x,y,z,w,h,q,l,p]=specs_spindles(Sig_par_single,timeasleep,fn,0);
fi_single_par(k)=x;
fa_single_par(k)=y;
amp_single_par(k)=z;
...
[x,y,z,w,h,q,l,p]=specs_spindles(Sig_cortex,timeasleep,fn,0);
fi_cohfo_cortex(k)=x;
fa_cohfo_cortex(k)=y;
...
[x,y,z,w,h,q,l,p]=specs_spindles(Sig_cortex_single,timeasleep,fn,0);
...
All_Par_26=[A_cell{:}];
specs_spindles(All_Par_26,timeasleep,fn,1)
...
%% Counts of spindles, cooccurrent spindles and single spindles.
    ...
    TT.Variables=    [[{['Count_' xx{1} '_total']};{['Count_' yy{1} '_total']};{['Count_coocur_' xx{1} '_' yy{1}]};{['Count_single_' xx{1}]};{['Count_single_' yy{1}]}] num2cell([...
        spindles_cortex;spindles_par;count_cohfo_par;count_single_cortex;count_single_par])];
...
%% Rates of spindles, cooccurrent spindles and single spindles.
    TT=table;
    TT.Variables=    [[{['Rate_' xx{1} '_total']};{['Rate_' yy{1} '_total']};{['Rate_coocur_' xx{1} '_' yy{1}]};{['Rate_single_' xx{1}]};{['Rate_single_' yy{1}]}] num2cell([...
        spindles_cortex_rate;spindles_par_rate;rate_cohfo_par;rate_single_cortex;rate_single_par])];

    TT.Properties.VariableNames=['Metric';cellfun(@(equis) strrep(equis,'_','-'),g,'UniformOutput',false).'].';

            writetable(TT,strcat(xx{1},'_',yy{1},'_spindles_rate','.xls'),'Sheet',1,'Range','A2:L10')    
            
%% PFC spindles features
    %All detections cortex
    TT=table;
    TT.Variables=    [[{'Count'};{'Rate'};{'Duration'};{'Average Frequency'};{'Instantaneous Frequency'};{'Amplitude'}] num2cell([spindles_cortex;spindles_cortex_rate;spindles_cortex_duration;fa_cortex;fi_cortex; amp_cortex])];    
    TT.Properties.VariableNames=['Metric' cellfun(@(equis) strrep(equis,'_','-'),g,'UniformOutput',false)].';
...
%All detections
    TT=table;
    TT.Variables=    [[{'Count'};{'Rate'};{'Duration'};{'Average Frequency'};{'Instantaneous Frequency'};{'Amplitude'}] num2cell([spindles_par;spindles_par_rate;spindles_par_duration;fa_par;fi_par;amp_par])];        
    TT.Properties.VariableNames=['Metric' cellfun(@(equis) strrep(equis,'_','-'),g,'UniformOutput',false)].';
